...
Usage:
  python paddle_ocr.py --input ../单词.pdf --output outputs/paddle.json --dpi 150 --max-pages 3
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="PaddleOCR — Chinese / English OCR via PaddleOCR"
    )
    ...
    try:
        ...
        print("Error: PaddleOCR not installed. Run:  pip install paddleocr paddlepaddle", file=sys.stderr)
        print(f"Details: {exc}", file=sys.stderr)
        sys.exit(1)

    print("Initialising PaddleOCR (this may take a moment) …")
    ...
    for page_idx in range(max_pages):
        ...
        # Write to temp PNG — PaddleOCR handles image paths reliably.
        ...
        # PaddleOCR new API returns OCRResult with rec_texts/rec_scores/rec_polys
        lines: list[dict] = []
        if raw and len(raw) > 0:
            ...
            for i in range(len(rec_texts)):
                ...
                poly = rec_polys[i]  # ndarray shape (4, 2)
                bbox_pts = [[float(p[0]), float(p[1])] for p in poly]
                x = bbox_pts[0][0]
                y = bbox_pts[0][1]
                w = bbox_pts[2][0] - bbox_pts[0][0]
                h = bbox_pts[2][1] - bbox_pts[0][1]
